...
**Teilfortschritt 2026-03-20:** Multi-Cam-Kalibrierung V1 umgesetzt. `src/cv/camera_calibration.py` haertet den ChArUco-Collector jetzt ueber Mindest-Ecken, Schaerfe und Reject-Gruende; `src/cv/stereo_calibration.py` ergaenzt `estimate_intrinsics()`, Board-Pose-Schaetzung und einen getrennten provisorischen Stationaer-Pfad. `src/web/routes.py` fuehrt `mode`/`capture_mode`, manuellen Frame-Capture, additive Readiness-Felder (`ready_full`, `ready_provisional`) und provisorische Stereo-Metadaten ein, waehrend `src/utils/config.py` das bestehende `pairs`-Schema nur additiv erweitert. `templates/index.html`, `static/js/app.js` und `static/css/style.css` zeigen jetzt Capture-Modus fuer Lens Setup, Handheld-vs-Stationaer fuer Stereo, Schaerfe/Reject-Status, manuelle Frame-Aufnahme und `Provisorisch`-Kennzeichnung im Ergebnis und Setup-Guide. Verifikation: `node -c static/js/app.js`, `python -m pytest tests/test_charuco_progress.py tests/test_routes_extra.py tests/test_stereo_wizard_api.py tests/test_multi_hardening.py tests/test_wizard_flow.py -q`, `python -m pytest tests/test_collector_quality.py tests/test_provisional_stereo.py tests/test_multi_cam_config.py -q`. Geaenderte Dateien: `src/cv/camera_calibration.py`, `src/cv/stereo_calibration.py`, `src/utils/config.py`, `src/web/routes.py`, `templates/index.html`, `static/js/app.js`, `static/css/style.css`, `tests/test_charuco_progress.py`, `tests/test_routes_extra.py`, `tests/test_multi_cam_config.py`, `tests/test_multi_hardening.py`, `tests/test_stereo_wizard_api.py`, `tests/test_wizard_flow.py`, `tests/test_collector_quality.py`, `tests/test_provisional_stereo.py`.

**Teilfortschritt 2026-03-20:** Der noch offene Start-to-Finish-Einstieg fuer den Multi-Cam-Wizard ist jetzt direkt im Setup-Guide verankert. `templates/index.html` bietet im laufenden Multi-Cam-Setup einen expliziten CTA mit Moduswahl (`Kalibrierboard bewegen` vs. `Kalibrierboard bleibt fest`) und einem separaten Direktpfad zur Stereo-Kalibrierung. `static/js/app.js` baut daraus jetzt eine echte Task-Queue aus `/api/multi/readiness`, laeuft fehlende Schritte pro Kamera in Reihenfolge ab (Lens nur im Handheld-Modus, dann Board, dann Stereo fuer das ausgewaehlte Paar), synchronisiert Entry-/Wizard-/Stereo-Modus miteinander und fuehrt den Benutzer nach `Back`/`Abbrechen` sauber in die Config-Ansicht zurueck. Im Auto-Capture-Fall startet der Wizard die Lens-Kalibrierung nach genuegend ChArUco-Frames jetzt auch ohne Extra-Klick. Verifikation: `node -c static/js/app.js`, `python -m pytest tests/test_wizard_flow.py tests/test_stereo_wizard_api.py -q`. Geaenderte Dateien: `static/js/app.js`, `templates/index.html`.
...
**Teilfortschritt 2026-03-20:** V1 des Wizards ist in die bestehenden Kalibrierungsdialoge integriert, nicht als separates Modul. Handheld und Stationaer sind jetzt explizit getrennt: Handheld nutzt weiter den vollen `stereoCalibrate()`-Pfad, Stationaer speichert einen klar markierten `provisional`-Datensatz ueber Board-Posen und geschaetzte Intrinsics. Der Wizard-/Dialog-Kontext zeigt Modus, Schaerfe, Reject-Gruende, manuelle Frame-Aufnahme und Ergebnis-Badges (`Kalibriert`/`Provisorisch`). Der Setup-Guide besitzt jetzt einen expliziten Wizard-CTA mit Task-Queue ueber die fehlenden Lens-/Board-/Stereo-Schritte; offene Folgearbeit fuer P29 ist damit nur noch Feinschliff an Quality Gates und optionalem Live-Progress, nicht mehr der fehlende Wizard-Einstieg selbst.
